...
description: >-
  ...
  testes do modulo. Use quando o usuario pedir para criar/adicionar um novo
  modulo de negocio (ex.: auth, transaction) dentro da pasta modules.
---
...
passos manualmente. Sempre invoque o script. Os arquivos do modulo sao
copiados literalmente da pasta [`assets/`](assets) (fonte unica de verdade da
estrutura pre-definida).
...
4. **Idempotente nos ajustes de config**: cada ajuste (ts-node, workspaces,
   dependencia nos apps) verifica se ja esta presente antes de escrever de
   novo; os 5 arquivos do modulo sao sempre sobrescritos com a versao de
   `assets/` (garante consistencia se a skill rodar de novo).
5. **Instalacao, build e teste automaticos**: ao final, roda `npm install` na
   raiz, `npm run build` na raiz (via Turborepo) e `npm test` dentro do
   modulo criado.
6. **Verificacao final**: confere que todos os arquivos do modulo existem, o
   `package.json` do modulo tem o `name` correto, `workspaces` e
   `devDependencies` do raiz estao corretos e as dependencias foram
   adicionadas em frontend/backend. Se algo falhar, o script termina com
   codigo de saida != 0 e lista o que falhou.
...
6. Roda `npm install` na raiz do projeto.
7. Roda `npm run build` na raiz do projeto.
8. Roda `npm test` dentro de `modules/<nome-do-modulo>`.
9. Roda a verificacao final descrita acima.
